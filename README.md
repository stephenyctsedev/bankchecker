...
Amount detection: the **second-to-last** decimal number on the line is taken as the transaction amount; the last is the running balance. If only one decimal appears (e.g. Firstrade lines without a balance column), that single value is taken as the amount.

> **Firstrade note:** The exact column layout depends on the statement version generated by Apex Clearing. If interest rows are missed or amounts are wrong, enable **Force OCR** in the sidebar and check the browser console for `[EXTRACT]` debug lines to see what raw text is being parsed.
